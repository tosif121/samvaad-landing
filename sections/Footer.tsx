...
const SocialIcon = ({ href, label, children }: { href: string; label: string; children: React.ReactNode }) => (
  <a
    ...
    aria-label={label}
    className="w-12 h-12 rounded-xl border border-black/10 flex items-center justify-center text-black/40 hover:text-green-600 hover:border-green-500/40 hover:bg-green-500/5 active:scale-90 transition-all duration-300"
  >
    {children}
  </a>
);
...
export default function Footer() {
  ...
  return (
    <footer className="px-6 md:px-12 py-12 md:py-24 bg-[#F4F4F4] overflow-hidden border-t border-black/10">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-12 mb-12 md:mb-20">
          {/* Brand */}
          <div className="col-span-2 md:col-span-2">
            ...
            <p className="text-black/55 text-sm leading-relaxed mb-6 max-w-sm font-medium">
              Samvaad is a high-performance AI Voice Workforce built by Iotcom.io. We help businesses never miss a lead
              again with human-like, 24/7 automated calling.
            </p>

            <div className="flex flex-col gap-3 mb-8">
              <a
                href={`tel:${SITE.phone.replace(/\s/g, '')}`}
                className="flex items-center gap-3 text-black font-bold hover:text-green-600 transition-colors"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-black/55 text-sm hover:text-black transition-colors"
              >
                {SITE.url}
              </a>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between border-t border-black/10 pt-10 gap-6">
          ...
          >
            <span className="w-8 h-8 rounded-full bg-black/5 flex items-center justify-center group-hover:bg-green-500 group-hover:text-black transition-all">
              ↑
            </span>
            Back to top
          </button>
